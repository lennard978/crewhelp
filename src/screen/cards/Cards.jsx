...
export const Cards = ({ data }) => {
	...
	const german = data[userId].map((item, index) => {
		return <Text key={index}>{item.german}</Text>;
	});
	const english = data[userId].map((item, index) => {
		return <Text key={index}>{item.english}</Text>;
	});
